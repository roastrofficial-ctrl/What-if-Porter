...
mkdir($root . '/acceptances', 0777, true);
file_put_contents($root . '/acceptances/' . $returnId . '.json', json_encode(['protocol' => 'PORTER/1', 'kind' => 'REMOTE_ACCEPTANCE', 'acceptance' => 'AC-' . bin2hex(random_bytes(16)), 'recipient' => 'sender', 'package' => $return, 'package_digest' => 'fixture', 'accepted_at_ms' => (int)round(microtime(true) * 1000)]));
...
assert(str_starts_with($collected['collection'], 'CL-'));
assert(is_file($root . '/collections/facts/' . $collected['collection'] . '.json'));
assert(!is_file($root . '/inbox/' . $returnId . '.json'));
unlink($root . '/collected/' . $returnId . '.json');
$client = new CollectionTickets($root, 'sender');
assert(is_file($root . '/collected/' . $returnId . '.json'), 'Collection fact did not recover Host projection');
