...
class RemoteRefusal(RuntimeError):
    def __init__(self, evidence):
        super().__init__(evidence.get("reason", "CORRESPONDENCE_NOT_ADMITTED")); self.evidence=evidence
...
class Porter:
    def __init__(self, identity, ipc, routes, transport=None, relationships=None, require_introductions=False, max_wire_bytes=262144):
        ...
        self.crash_after_response_once=False;self.max_wire_bytes=max_wire_bytes
        ...
        self.admission=Admission(self.ipc,self.identity,relationships or {},require_introductions)
    ...
    def _network_deposit(self,value,route):
        ...
        request=Request(route.rstrip("/")+"/deposit",json.dumps(body,separators=(",",":")).encode(),{"Content-Type":"application/json"},method="POST")
        try:return json.load(urlopen(request,timeout=10))
        except HTTPError as exc:
            try:evidence=json.load(exc)
            except Exception:raise
            if evidence.get("kind")=="REFUSE":raise RemoteRefusal(evidence)
            raise
    ...
    def carry_one(self, path, fail_after=None):
        ...
        try:
            ...
            atomic_json(self.ipc/"refused"/f"{value['package']}.json",refusal.evidence)
            claimed.unlink(missing_ok=True)
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if not wire_size_allowed(length,porter.max_wire_bytes):
                ...
                return self.reply(413,{"protocol":"PORTER/1","kind":"REFUSE","state":"POLICY_REFUSED_BEFORE_ACCEPTANCE","reason":"CORRESPONDENCE_NOT_ADMITTED"})
            try:
                body=json.loads(self.rfile.read(length));wrapped=isinstance(body,dict) and "package" in body
                self.reply(202,porter.deposit(body["package"] if wrapped else body,admission=body.get("admission") if wrapped else None))
            except AdmissionRefused as exc:self.reply(403,{"protocol":"PORTER/1","kind":"REFUSE","state":"POLICY_REFUSED_BEFORE_ACCEPTANCE","reason":exc.public_reason})
            except Exception as exc:self.reply(400,{"protocol":"PORTER/1","kind":"REFUSE","state":"INVALID_ENVELOPE","reason":str(exc)})
    return Handler
...
def main():
    ...
    parser.add_argument("--relationships",default="{}");parser.add_argument("--require-introductions",action="store_true");parser.add_argument("--max-wire-bytes",type=int,default=262144)
    args=parser.parse_args();porter=Porter(args.identity,args.ipc,json.loads(args.routes),relationships=json.loads(args.relationships),require_introductions=args.require_introductions,max_wire_bytes=args.max_wire_bytes);porter.crash_after_response_once=args.experiment_crash_before_acceptance_evidence;threading.Thread(target=porter.carry,daemon=True).start();host,port=args.listen.rsplit(":",1);ThreadingHTTPServer((host,int(port)),handler(porter)).serve_forever()
